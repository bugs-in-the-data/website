...
def main():
    ...
    for sample in samples:
        sample_id = sample[0]
        utm_easting = float(sample[1]) if is_number(sample[1]) else "NULL"
        utm_northing = float(sample[2]) if is_number(sample[2]) else "NULL"
        zone = float(sample[3][:2]) if is_number(sample[3][:2]) else "NULL"
